...
This statement overrides the timeout programmatically and takes precedence over the configuration:
...
We recommend trying execution profiles first if you can identify static categories of statements that share the same options.
...
In fact, some queries explicitly require no graph name, such as those that access the `system` query.
...
datastax-java-driver {
  ...
  profiles {
    graph-system {
      # Do not inherit the graph name here
...
There is one exception: if you use the xref:core:dse/graph/script-api.adoc[script API] against a legacy DSE version (5.0.3 or older), the driver infers the wrong protocol version.
