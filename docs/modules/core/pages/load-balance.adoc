...
xref:core:connection-pool.adoc[Connection pools] can be sized independently for each distance.
...
The actual number of connections to open to each one of those nodes is configurable, see xref:core:connection-pool.adoc[Connection pools] for more details.
...
First make sure that xref:core:metadata/metadata.adoctoken/#configuration[token metadata] is enabled.
